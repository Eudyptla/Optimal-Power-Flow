function [h,g,dh,dg] =  NlinFcn_Ploss_Lindex_maxvoltageprofile(x,bus,gen)
% ====================================================================
...
Lindex = abs(Lj);
h1 = Lindex-x(nvars-1)*ones(nb-ng,1);
nh1=size(h1, 1);
Voltage_Dev = Vm(idx_L)-ones(nb-ng,1);
Voltage_Profile= abs(Voltage_Dev);
h2=Voltage_Profile-x(nvars)*ones(nb-ng,1);
nh2=size(h2, 1);
h=[h1;h2];
