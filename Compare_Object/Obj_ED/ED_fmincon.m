function Obj_ED = ED_fmincon(mpc)
...
gh_fcn = @(x) NlinFcn_ED_3(x,bus,gen);

% ====================================================================
% Hessian matrix
% ====================================================================  
cost_mult = 1;   % For interior-point algorithm
hess_fcn = @(x,lambda) HessFcn_ED_3(x,lambda,cost_mult);
...
Qcost= sum( coeff2.*((PQ./S2).^2).*(Qg.^2)+coeff1.*(PQ./S2).*Qg+coeff0);
 ...
 Obj_ED = [Pcost,Ploss,Voltage_Profile,Lindex_max,QReserve...
                  Qcost];
